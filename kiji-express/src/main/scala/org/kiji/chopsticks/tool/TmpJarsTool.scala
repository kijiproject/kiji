...
package org.kiji.chopsticks.tool
 ...
 * distributed cache of Hadoop jobs run through KijiChopsticks. It should be used from the `chop`
 * script and should be passed a classpath entry for the KijiChopsticks `lib` directory as well
 * as any classpath entries specified by the user through `CHOP_CLASSPATH`.
 *
