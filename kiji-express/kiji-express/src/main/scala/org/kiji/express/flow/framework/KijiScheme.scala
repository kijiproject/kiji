...
class KijiScheme(
  ...
  override def sourceConfInit(
    ...
    val request: KijiDataRequest = withKijiTable(uri, conf) { table =>
      BaseKijiScheme.buildRequest(table.getLayout, timeRange, inputColumns.values)
    }
      ...
      cf.pagingSpec match {
        ...
        case PagingSpec.Cells(pageSize) => {
          def genItr(): Iterator[FlowCell[_]] = {
            new MapFamilyVersionIterator(row, cf.family, BaseKijiScheme.qualifierPageSize, pageSize)
                .asScala
