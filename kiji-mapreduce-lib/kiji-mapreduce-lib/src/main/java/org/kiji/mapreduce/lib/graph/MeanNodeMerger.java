...
/**
 * Merge nodes using mean.
 */
public class MeanNodeMerger extends AggregateNodeMerger {
  /** The node count. */
  ...
  protected void update(Node mergedNode, Node newNode) {
    mergedNode.setWeight(mergedNode.getWeight() + newNode.getWeight());
    mNodeCount++;
  }
  ...
  protected void close(Node mergedNode) {
    if (0 == mNodeCount) {
      ...
      mergedNode.setWeight(mergedNode.getWeight() / mNodeCount);
    }
  }
}
