...
non-distributed java programs. Both types of programs use [`KijiTableWriter`]({{site.api_schema_1_1_0}}/KijiTableWriter.html)s to
delete data.
...
DeleteEntry uses a [`KijiTableWriter`]({{site.api_schema_1_1_0}}/KijiTableWriter.html) to perform point deletions on a kiji table:
...
Deletions from within a MapReduce job are also performed using a [`KijiTableWriter`]({{site.api_schema_1_1_0}}/KijiTableWriter.html).
...
A [`KijiTableWriter`]({{site.api_schema_1_1_0}}/KijiTableWriter.html) we opened in the `setup()` method is then used to delete
...
To learn more about Kiji, check out these other resources:
 - [User Guide]({{site.userguide_schema_1_1_0}}/kiji-schema-overview)
