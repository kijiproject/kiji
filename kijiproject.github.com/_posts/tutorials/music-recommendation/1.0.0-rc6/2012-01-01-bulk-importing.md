...
[text input format]({{site.userguide_mapreduce_rc6}}/command-line-tools/#input) where the
...
Then build an [Avro]({{site.userguide_mapreduce_rc6}}/working-with-avro) metadata record from the parsed JSON.
...
We create an [`EntityId`]({{site.api_schema_1_0_0}}/EntityId.html) object in order to use the song ID
as the row key.
