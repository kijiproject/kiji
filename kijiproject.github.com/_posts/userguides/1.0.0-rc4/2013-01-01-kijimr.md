...
This is a working copy of the Kiji MapReduce user guide. Please use markdown for formatting.

# Overview

Kiji MapReduce is the second major component in the Kiji ecosystem for Apache Hadoop and HBase after KijiSchema.
KijiMR primarily includes APIs to build MapReduce jobs that run over Kiji tables, bringing MapReduce-based analytic techniques to a broad base of KijiSchema users, eg. to apply machine learning techniques.
In addition to that, Kiji MapReduce Library brings a repository of real world examples that can be used as the basis for these tasks.

KijiMR is organized around three core MapReduce job types: _Bulk Importers_, _Producers_ and _Gatherers_, that may be combined with external data sources through _KeyValueStores_.
_Bulk Importers_ load data from external stores such as files in HDFS clusters into Kiji tables.
_Producers_ are entity-centric operations whose purpose is to generate new content within an entity and based on the entity's state. One typical use-case for producers is to generate new recommendations for a user based on the user's history.
_Gatherers_ provide full MapReduce computations on Kiji tables by scanning over Kiji table rows. Gatherers may also export data to external data stores such as files in HDFS clusters.

Finally, KijiMR allows any of these jobs to combine their data with external _KeyValueStores_ as a way to join data sets.
...
There are two options for importing data into Kiji tables: using a bulk importer or "putting" individual rows. A bulk importer is suitable for reading in data from files, whereas putting is suitable for importing individual rows. Kiji MapReduce Lib contains a set of sample parsers and bulk importers that enable users to import data for many common data formats (including CSV, JSON, and the Apache Common Log Format).

### Classes Overview

Kiji bulk importers rely on two classes:
* _KijiBulkImporter_ - the base class which concrete bulk importers.  Subclasses implement the produce method for converting the raw data into Kiji puts, as well as the setup and teardown methods for doing any sort of relevant configuration.
...
If one of the precanned bulk importers listed above is insufficient for the data that is to be imported, the KijiBulkImporter or the DescribedTextBulkImporter class can be extended to support parsing and importing the desired data.  The produce method needs to be implemented to handle the extraction of data from the import text.

Once you have the requisite bulk-importer, importing the data can simply follow these steps:
* Define any necessary configuration for the bulk importer.  For example, the bulk importers in Kiji MapReduce Library that use the DescribedInputTextBulkImporter all require a table import descriptor to map between source data and destination data.
* Once the proper configuration file has been written, the data can be bulk imported into Kiji via commands like:
kiji bulk-import --kiji=${KIJI} \
 ...
 * by the Hadoop text input format, where the key is the offset of the line in the input
 * file as a LongWritable named 'filePos', and the value is the line content, as a Text
 * object named 'value'.
 */
...
A Kiji Gatherer scans over the rows of a Kiji table using the MapReduce framework to aggregate information which can be passed to a Reducer. For common tasks, you may use one of the Gatherers that ship with Kiji MapReduce. For example, you may use a PopGatherer to compute the popularity of a set of products purchased in your customer table, or a CartesianProductGatherer to count co-occurrences of web page visits within your session table.

### Classes Overview

All gatherers extend KijiGatherer, which is a MapReduce Mapper. The Kiji Gather framework takes care of configuring an appropriate MapReduce job and connecting gatherers to the right inputs and outputs. The three most important methods of a KijiGatherer are:

* `Class<?> getOutputKeyClass()` and `Class<?> getOutputValueClass()` : specify the classes of output keys and values emitted by the gatherer.

* `KijiDataRequest getDataRequest();` : specifies the columns this gatherer requires.

* `public void setup(GathererContext<K, V> context) throws IOException` : called exactly once before any calls to the gather() operation, to initialize the gatherer.

* `public void gather(KijiRowData input, GathererContext<K, V> context) throws IOException;` : called once for each input row. .

* `public void cleanup(GathererContext<K, V> context) throws IOException` : called exactly once when the gatherer is done to finalize the task.
...
A KijiProducer executes a function over a subset of the columns in a table row and produces output to be injected back into a column of that row. Producers can be run in the context of a MapReduce over entire Kiji tables, or on-demand over a single row at a time. Common tasks for producers include parsing, profiling, recommending, predicting, and classifying. For example, you might run a LocationIPProducer to compute and store the location of each user into a new column, or a PersonalizationProfileProducer to compute a personalization profile.

### Classes Overview

All producers extend `KijiProducer`. The most important methods of a KijiProducer are:

* `KijiDataRequest getDataRequest()` : specifies the columns this producer requires from the input table.

* `void produce(KijiRowData input, ProducerContext context) throws IOException` :  called once for each input row.  It does any relevant processing and writes the output to the reduce phase.
...
public static class ProducerExample extends KijiProducer {
  ...
  public String getOutputColumn() {
    // Configure the producer to emit to column 'produced:recommendations':
    return "produced:recommendations";
  }
...
Kiji reducers extends the class `KijiReducer`. The most important methods in a
...
A  Kiji Transformer scans over a file in HDFS and uses the MapReduce framework to aggregate information which can be passed to a Reducer.  This is useful for analyzing data that already lives inside of HDFS, but does not live in Kiji.  This allows you to use the same Mappers and Reducers that are used in other Kiji jobs.

### Classes Overview

Transform jobs are created using the KijiTransformJobBuilder.  Transform jobs require more setup because their data does not exist in Kiji, and so the inputs and outputs need to be specified explicitly.  Inside of the KijiTransformBuilder, you’ll find the withMapper() and the withReducer methods to be able to explicitly specify your map and reduce tasks.  To specify the type of the input required for this, the withInput method is called

* withInput(MapReduceJobInput jobInput) - specifies the type of the input for a KijiTransformJobBuilder.  Possible examples are:
...
* `kiji job-history`: interact with the job history table.
...
* `--output=`: specifies the output of the job.
  ...
  Kiji recognizes the following job output formats:
    ...
    * `kiji`: job output is a Kiji table. The use of this job output should be limited to development only and should not be used in production as it may incur high load on the target HBase cluster.
    ...
    * `text`: job output is a text file; each (key, value) record is written to a text file with a separator (via the configuration parameter "mapred.textoutputformat.separator", which defaults to TAB) and a new line.
...
Kiji unit tests inherit from the base class `KijiClientTest` which provides an environment suitable for tests of Kiji schema logic and MapReduce logic. It provides a testing Hadoop configuration accessible through `KijiClientTest.getConf()`. The KijiClientTest base class keeps track of Kiji instances created for testing. In particular, it provides a default Kiji instance accessible with `KijiClientTest.getKiji()`. Other Kiji instances may be created with `KijiClientTest.createKiji()`.
...
Thw `RequiredStores.with()` method will return a `Map` object augmented with a `with(String, KeyValueStore)` method, so you can call `RequiredStores.with("foo", store1).with("bar", store2);
