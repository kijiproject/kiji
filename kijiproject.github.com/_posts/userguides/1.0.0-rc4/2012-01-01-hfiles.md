...
When using direct writes, the `nsplits` parameter is ignored.
...
Kiji MapReduce jobs are configured to write HFiles through the `format=hfile` job output specification on the command-line,
