...
classes are accessible either in `$KIJI_CLASSPATH` or in the `kiji-rest-0.5.0/lib/` directory.
...
You can find the process ID in the `kiji-rest.pid` file:
    ...
    ~/kiji-rest-0.5.0$  cat kiji-rest.pid
    1234
...
1. Unpack the `kiji-rest` tarball as the `/opt/wibi/kiji-rest directory`:
        ...
        $ tar -xzf kiji-rest-0.5.0-release.tar.gz -C /opt/wibi/
        $ ln -s kiji-rest-0.5.0 kiji-rest
