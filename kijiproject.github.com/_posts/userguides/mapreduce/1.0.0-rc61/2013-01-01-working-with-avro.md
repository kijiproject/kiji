...
Likewise, an `org.apache.mapred.AvroValue` may be used for Avro data as the output value. Implement the [`AvroValueWriter`]({{site.api_mr_rc61}}/avro/AvroValueWriter.html) interface to specify the writer schema. To use Avro data as your bulk importer, mapper, or reducer's input key or value, wrap it in an `AvroKey` (or `AvroValue` for values) and implement [`AvroKeyReader`]({{site.api_mr_rc61}}/avro/AvroKeyReader.html) (or [`AvroValueReader`]({{site.api_mr_rc61}}/avro/AvroValueReader.html)) to specify the reader schema.
