...
Using the job builder API, this is what the job output class [`DirectKijiTableMapReduceJobOutput`]({{site.api_mr_1_0_0_rc62}}/output/DirectKijiTableMapReduceJobOutput.html)
...
Under the hood, the output format used by each MapReduce tasks will open a [`KijiTableWriter`]({{site.api_schema_1_0_3}}/KijiTableWriter.html)
...
Using the job builder API, the HFile job output is implemented by the [`HFileMapReduceJobOutput`]({{site.api_mr_1_0_0_rc62}}/output/HFileMapReduceJobOutput.html) job output class,
