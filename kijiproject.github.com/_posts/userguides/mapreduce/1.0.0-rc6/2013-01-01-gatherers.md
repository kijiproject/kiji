...
All gatherers must extend the parameterized class [`KijiGatherer`]({{site.api_mr_rc6}}/gather/KijiGatherer.html) with the types
...
[`KeyValueStore`]({{site.api_mr_rc6}}/kvstore/KeyValueStore.html)s by implementing `Map<String,
KeyValueStore<?, ?>> getRequiredStores()`. This method should construct and return a map specifying
all the [`KeyValueStore`]({{site.api_mr_rc6}}/kvstore/KeyValueStore.html)s that the gatherer wants
to access. The [`KeyValueStore`]({{site.api_mr_rc6}}/kvstore/KeyValueStore.html)s may then later be
accessed from the `gather()` method through the
[`GathererContext`]({{site.api_mr_rc6}}/gather/GathererContext.html). For more details, you may
check the [KeyValue Stores]({{site.userguide_mapreduce_rc6}}/key-value-stores) section in this guide.
...
[`KeyValueStore`]({{site.api_mr_rc6}}/kvstore/KeyValueStore.html), it should be opened once with `context.getStore(storeName)` in
