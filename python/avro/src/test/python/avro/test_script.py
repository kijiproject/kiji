...
def get_avro_tool_path():
    """Returns: the path of the avro-tool executable.

    It is assumed here that the avro-tool executable is generated in the same folder
    of the test executable.
    """
    this = os.path.abspath(sys.argv[0])
    avro_tool_path = os.path.join(os.path.dirname(this), "avro-tool")
    assert os.path.exists(avro_tool_path), \
        ("Avro script not found: {!r}".format(avro_tool_path))
    return avro_tool_path


def run_avro_tool(*args, stdin=None):
    command = [get_avro_tool_path()]
    command.extend(args)
    env = dict(os.environ)
...
class TestCat(unittest.TestCase):
    ...
    def setUp(self):
        ...
        delete = True
        self._avro_file = tempfile.NamedTemporaryFile(prefix="test-", suffix=".avro", delete=delete)
    ...
    def _RunCat(self, *args, raw=False):
        ...
        """
        out = run_avro_tool("cat", self._avro_file.name, *args)
        if raw:
            return out
        else:
    ...
    def testVersion(self):
        out = run_avro_tool("cat", "--version").decode("utf-8")
...
class TestWrite(unittest.TestCase):
    ...
    def _RunWrite(self, *args, stdin=None):
        ...
        """
        return run_avro_tool(
            "write", "--schema", self._schema_file.name,
            stdin=stdin, *args
        )

    def LoadAvro(self, filename):
        out = run_avro_tool("cat", filename).decode("utf-8")
        return tuple(map(json.loads, out.splitlines()))

    def testVersion(self):
        out = run_avro_tool("write", "--version").decode("utf-8")
