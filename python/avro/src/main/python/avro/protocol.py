...
"""Representation of Avro protocols."""
...
class Protocol(object):
    ...
    def _parse_message_desc(name, message_desc, names):
        ...
        if request_desc is None:
            raise ProtocolParseException('Invalid message descriptor with no "request": %r.'
                                         % message_desc)
        ...
        if response_desc is None:
            raise ProtocolParseException('Invalid message descriptor with no "response": %r.'
                                         % message_desc)
...
def protocol_from_json_data(json_data):
    ...
    if type(json_data) != dict:
        raise ProtocolParseException('Invalid JSON descriptor for an Avro protocol: %r' % json_data)

    name = json_data.get('protocol')
    if name is None:
        raise ProtocolParseException('Invalid protocol descriptor with no "name": %r' % json_data)
    ...
    types = tuple(map(lambda desc: Protocol._parse_type_desc(desc, names=names), type_desc_list))
...
def parse(json_string):
    ...
    Returns:
        The parsed protocol.
    Raises:
        ProtocolParseException: on JSON parsing error, or if the JSON descriptor is invalid.
    """
    try:
