...
/**
 * Factory for Hadoop 1.x-specific KijiMRPlatformBridge implementation.
 */
@ApiAudience.Private
public final class Hadoop1xKijiMRBridgeFactory extends KijiMRPlatformBridgeFactory {
    ...
    try {
      Class<? extends KijiMRPlatformBridge> bridgeClass =
          (Class<? extends KijiMRPlatformBridge>) Class.forName(
              "org.kiji.mapreduce.platform.Hadoop1xKijiMRBridge");
  ...
  public int getPriority(Map<String, String> runtimeHints) {
    ...
    if (hadoopVer.matches("1\\.0\\.[0-9\\.]+")) {
      // Hadoop 1.0.x matches correctly; use this bridge.
